...
function constructorSistemaMateria($sistema,$materia,$fechaInicio,$fechaFinal){
    $thisSistema=strtoupper ($sistema);
    $thisMateria=strtoupper ($materia);
    $thisFechaInicio='';
    $thisFechaFinal='';
    ...
    if(($fechaFinal!=""|$fechaFinal!=" ")&($fechaFinal!=" "|$fechaFinal!=""))
          ...
          realizandoOperacionSistemaMateria($thisSistema,$thisMateria,$thisFechaInicio,$thisFechaFinal);
 
}
function realizandoOperacionSistemaMateria($sistema,$materia,$fechaIncio,$fechaFinal){
    //principalSistema('ORAL','',''); // PRIMERO CREAMOS LA VISTA EL CUAL SERA CONSUMIDO 
    principalSistemaMateria($sistema,$materia,$fechaIncio,$fechaFinal); // PRIMERO CREAMOS LA VISTA EL CUAL SERA CONSUMIDO 
  //print_r();
    //$preguntas=preguntasMateria(10);
   // $preguntas=preguntasSistema('ORAL');// SIEMPRE MANDARLO EN MAYUSCULAS
    $preguntas=preguntasSistemaMateriaAgrupadas($materia,$sistema);// SIEMPRE MANDARLO EN MAYUSCULAS
    $equals="";
    //print_r($preguntas);
    if($preguntas!=0)
    foreach ($preguntas as $key => $value) {
      if($value['pregunta']!=$equals){
       ...
       // $array['datosGenerale']=fintrarPorPregunta($value['pregunta'],10);
        $Arrayvalor=filtrarPorPreguntaSistemaMateria($value['pregunta'],' ',' ');
       // print_r($Arrayvalor);
...
//print_r( $respuesta);
function getRespuestaSistemaMateria(){
    global $respuestaOperacion;
